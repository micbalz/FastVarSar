...
run = function(n, W, q) {
  lambda_t = 0.2
  beta_t = c(1, 4, -2.5, -3.5, 3, rep(0,q))
...
sims = function(Q, nsim) {
  ...
  for (q in Q) {
    ...
    cat("Running simulation study for q =", q, "\n")
    
    pb = progressor(along = 1:nsim)
    
    results_for_q = future_lapply(1:nsim, function(i) {
      
      res = run(n, W, q)
      pb(sprintf("q=%.2f, replication %d", q, i))
      res
    }, future.seed = TRUE)
    
    result[[paste0("q=", q)]] = results_for_q
  }
  return(result)
}

Q = c(96, 196, 296, 396, 496)
results = sims(Q, nsim)
