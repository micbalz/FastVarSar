...
pacman::p_load(tidyverse, data.table, knitr, Matrix, glmnet, mboost, gamboostLSS, MASS, spdep, spatialreg, furrr, future, future.apply, progressr)
...
run = function(n, lambda_t, beta_t, sigma_t, rho) {
  ### Generate train data
  ...
  ### Generate test data
  X_test = MASS::mvrnorm(n, mu = rep(0, p), Sigma = Sigma)
...
sims = function(rho_values, nsim) {
  ...
  for (rho in rho_values) {
    ...
    cat("Running simulation study for rho =", rho, "\n")
    
    pb = progressor(along = 1:nsim)
    
    results_for_rho = future_lapply(1:nsim, function(i) {
      
      res = run(n, lambda_t, beta_t, sigma_t, rho)
      pb(sprintf("rho=%.2f, replication %d", rho, i))
      res
    }, future.seed = TRUE)
    
    result[[paste0("rho=", rho)]] = results_for_rho
  }
  return(result)
}

rho_values = c(0, 0.2, 0.4, 0.6, 0.8)
results = sims(rho_values, nsim)
